...
const zooAnimals = [
  ...
  */
  ...
  {
    let displayNames = []; // here's your object

    zooAnimals.forEach(function(item,index) 
    {
        let animal = `name: ${item.animal_name}, scientific: ${item.scientific_name}`

      displayNames.push(animal); // you push it to the array
    });  
    return displayNames;
  }
  ...
  */
  ...
  function lowerCaseNames(zooAnimals){
    let reformatNames = zooAnimals.map(a => (`${a.animal_name.toLowerCase()}`));
  ...
  */
  ...
  function lowPopulationAnimals(zooAnimals){
    return zooAnimals.filter(animals => animals.population < 5);
  }
  ...
  */
  ...
  function USApop(zooAnimals){
    return zooAnimals.reduce((accumulator, animals) => accumulator + animals.population, 0);
  }
...
/* 🐴🐴🐴 Step 4: Create a new object that uses CuboidMaker (not auto graded)🐴🐴🐴
  ...
  let attributes = { length: 4, width: 5, height: 5};
...
let cuboidTwo = new CuboidMakerTwo(attributes);
